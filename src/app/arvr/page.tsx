...
// Configuration for Demo Assets (Verified Stable Links)
const assetsForDemo = [
  {
    name: "Robot (Cyber Vibe)",
    // Google's top-level stable asset
    glb: "https://modelviewer.dev/shared-assets/models/RobotExpressive.glb",
    usdz: undefined
  },
//   {
//     name: "The Duck (Small Palm Prop)",
//     // Properly hosted through Khronos Group's active web pages server (No CORS issues)
//     glb: "https://khronosgroup.github.io/glTF-Sample-Assets/Models/Duck/glTF-Binary/Duck.glb",
//     usdz: undefined
//   },
  {
    name: "Astronaut (Scale Demo)",
    // Google's most stable top-level asset
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pt-24 pb-12 px-4 md:px-8 mt-10 font-sans selection:bg-white/20">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-12 items-start">
        ...
        >
          {/* @ts-ignore - Bypassing TS error if global.d.ts isn't fully cached yet */}
          <model-viewer
            src={currentAsset.glb}
            ios-src={currentAsset.usdz}
            alt={`3D product model placeholder - ${currentAsset.name}`}
            ar
          ...
          <div className="absolute bottom-8 left-0 w-full flex flex-col items-center justify-center space-y-4 pointer-events-none">
            ...
            >
              ...
              <div className="flex items-center gap-1 text-black bg-white/50 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={isAutoRotate ? "animate-spin-slow" : ""}>
        ...
        >
          ...
          <div className="space-y-3 mb-8">
            ...
            <button 
              // This triggers the AR mode directly from the external button
              onClick={() => (document.querySelector('model-viewer') as any)?.activateAR()}
              className="w-full bg-transparent border border-white text-white font-bold py-4 rounded-full uppercase tracking-widest text-sm hover:bg-white/5 transition-colors"
            >
              Try on Now (AR)
            </button>
          </div>
