...
//  // 🚀 SEO FIX: Initialize active secondary filters directly from the URL query parameters
...
//           <div className="flex-1 min-w-0 flex gap-3 overflow-x-auto pb-1 lg:pb-0 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
//             <div className="flex gap-3 pb-1 lg:pb-0 w-max">
//             <button
...
function ShopAllContent() {
  ...
  // 🚀 REACT FIX: Ensure `router.push()` happens OUTSIDE of the setState call
  const toggleFilter = (group: keyof ActiveFilters | 'RESET', value?: string) => {
    if (group === 'RESET') {
      setActiveFilters({ collections: [], styles: [], sizes: [], placements: [] });
      router.push(pathname, { scroll: false });
    ...
    // 2. Safely Update State
    setActiveFilters(newState);

    // 3. Push URL params as a side effect
    const params = new URLSearchParams(searchParams.toString());
  ...
  return (
    // 🚀 STYLING FIX: Added `overflow-x-hidden w-full` to prevent mobile side-sway
