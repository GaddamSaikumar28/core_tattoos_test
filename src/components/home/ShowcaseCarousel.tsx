...
// // --- Sub-Components ---
// // 1. Added mobileHidden to the props
...
//       ref={ref}
//       className={clsx(
//         // 2. Hide on mobile if mobileHidden is true, but force flex on desktop
...
//            <button onClick={handleARClick} className="flex items-center text-[--color-brand-purple-light] text-[10px] uppercase font-bold tracking-wider hover:brightness-125 transition-all">
//              <ArIcon />
//              Try On AR
//            </button>
//         </div>
...
//   // 3. State to control how many items show on mobile initially
...
//         >
//           {isLoading ? (
//             /* Updated skeleton count to 6 to match the mobile initial view */
//             [...Array(6)].map((_, i) => (
...
//                   mode={mode} 
//                   index={index} 
//                   // 4. Pass the hidden state flag down to the card
//                   mobileHidden={index >= visibleMobileCount} 
//                 />
//               ))}

//               {/* 5. Show More Button (Only renders if there are extra items, only visible on mobile) */}
...
const CarouselCard = ({ item, mode, index, mobileHidden }: { item: FormattedProduct, mode: 'product' | 'collection', index: number, mobileHidden?: boolean }) => {
  ...
  useEffect(() => {
    ...
    // Cleanup camera stream when AR mode is closed
    return () => {
      if (stream) {
