...
function getCfg(isMobile: boolean): CarouselConfig {
  if (isMobile) {
    // CONFIDENT MOBILE SIZING (Avoids clipping, scales beautiful)
    return { cardW: 180, cardH: 260, radius: 170, perspective: 900,  rotSpeed: 0.025 };
  }
  // OPTIMIZED DESKTOP VIEW
  return { cardW: 280, cardH: 420, radius: 420, perspective: 1200, rotSpeed: 0.02 };
}
...
const CAROUSEL_CSS = `
  @keyframes cardFall {
    0%   { transform: translateY(var(--fall-from)) rotateZ(var(--fall-rotZ)) scale(0.85); opacity: 0; }
    15%  { opacity: 1; }
    70%  { transform: translateY(12px) rotateZ(calc(var(--fall-rotZ) * 0.15)) scale(1.05); opacity: 1; }
    85%  { transform: translateY(-4px) rotateZ(0deg) scale(0.98); opacity: 1; }
    100% { transform: translateY(0px) translateZ(var(--stack-z-offset, 0px)) rotateZ(0deg) scale(1); opacity: 1; }
  }
  /* GLARE EFFECT DISABLED FROM CSS BELOW */
  /* @keyframes glintSweep {
    0%   { transform: translateX(-140%) skewX(-18deg); }
    100% { transform: translateX(260%) skewX(-18deg); }
  }
...
function CylinderCarousel({
  ...
  const mouseParallax     = useRef({ x: 0, y: 0 });
  const phaseRef          = useRef<Phase>(shouldReduceMotion ? "rotate" : "fall");
  const lastActiveCardRef = useRef<number>(-1);

  // CRITICAL PERFORMANCE FIX: Keep an mutable reference of callback to stop frame drops
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => { setPhase("expand"); setShowRipple(true); setTimeout(() => setShowRipple(false), 1200); }, T_EXPAND);
    const t2 = setTimeout(() => { setPhase("tilt"); }, T_TILT);
    const t3 = setTimeout(() => { setPhase("rotate"); }, T_ROTATE);
    return () => { clearTimeout(t1); clearTimeout(t2); clearTimeout(t3); };
  }, [shouldReduceMotion]);
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      if (lastActiveCardRef.current !== activeCard) {
        lastActiveCardRef.current = activeCard;
        // Native non-blocking callback call via isolated reference tracking
        onSlideChangeRef.current(activeCard);
      }
    ...
    rafRef.current = requestAnimationFrame(loop);
    return () => { cancelAnimationFrame(rafRef.current); };
  }, [phase, cfg.radius, cfg.rotSpeed, isMobile]); // Safely removed onSlideChange dependency
    ...
    >
      ...
      <div
        style={{
          ...
          perspectiveOrigin: "50% 50%",
          display: "flex", alignItems: "center", justifyContent: "center",
          // Safely shifts position down 6.5vh on mobile devices to prevent bumping into headers
          transform: isMobile ? "translateY(6.5vh)" : "none",
        }}
      >
        <div ref={cylinderRef} style={{ position: "relative", width: "0px", height: "0px", transformStyle: "preserve-3d" }}>
          {displayCards.map((card, i) => {
            ...
            return (
              ...
              >
                <div className="relative w-full h-full">
                  ...
                  {/* GLARE ELEMENTS REMOVED/COMMENTED OUT OUT BELOW */}
